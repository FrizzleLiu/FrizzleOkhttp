 ...
 * 连接服务器的拦截器
 */
public class ConnectionServerInterceptor implements FrizzleInterceptor {

    private final String TAG = "conn";

    @Override
    public FrizzleResponse doNext(FrizzleChain frizzleChain) throws IOException {

        // 解析Reqeus
        SocketRequestServer srs = new SocketRequestServer();
        FrizzleRequest request = frizzleChain.getRequest(); // 更新后的Request  hostName:  post:leng type
